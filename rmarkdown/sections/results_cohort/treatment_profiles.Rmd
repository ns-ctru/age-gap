...
1. Be very strict and only compare those who have a unique single treatment in the categories defined.  This answers the direct comparison that seems to underping the original hypothesis being tested.  However it would result in a concomitant reduction in statistical power.
...
For the proposed longitudinal analyses I would personally advocate option 3, with each indicator being repeated in effect all starting as `0` (zero or negative) to indicate the treatment hadn't been received and then switching to `1` (one or positive) at the follow-up at which they indicate a given treatment has been received.  Baseline co-variates would simply be those which subject specific knowledge feel are important to adjust for (and would in essence simply be an additional time-point).  For individual follow-up times (i.e. **Primary Outcome** which is follow-up at six months, although I expect other time points to be of interest) I would suggest its a decision between option 1 and 3.  The later would maintain sample size and in turn power of the study as it was designed whilst adjusting for the treatments that are adjunct to those of interest.
