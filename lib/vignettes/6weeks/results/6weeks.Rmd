### 6 weeks Characteristics

6 weeks characteristics are summarised and tabulated below for each of the Primary Treatment Groups.

#### Age
```{r 6weeks_summary_demographics_age, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of Age at 6 weeks
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
...
test$continuous %>%
    ...
    kable(col.names = c("Metric", "Primary Treatment", "N", "Missing", "Mean (SD)", "Median (IQR)", "Range"),
          caption = "Age at 6 weeks by primary treatment group.")


```

#### Height
```{r 6weeks_summary_demographics_height, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of Age at 6 weeks
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
...
test$continuous %>%
    ...
    dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name, -label) %>%
    kable(col.names = c("Metric", "Primary Treatment", "N", "Missing", "Mean (SD)", "Median (IQR)", "Range"),
          caption = "Height at 6 weeks by primary treatment group.")

```

#### Weight
```{r 6weeks_summary_demographics_weight, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of Age at 6 weeks
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
...
test$continuous %>%
    ...
    dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name, -label) %>%
    kable(col.names = c("Metric", "Primary Treatment", "N", "Missing", "Mean (SD)", "Median (IQR)", "Range"),
          caption = "Weight at 6 weeks by primary treatment group.")

```

#### BMI
```{r 6weeks_summary_demographics_bmi, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of Age at 6 weeks
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
...
test$continuous %>%
    ...
    dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name, -label) %>%
    kable(col.names = c("Metric", "Primary Treatment", "N", "Missing", "Mean (SD)", "Median (IQR)", "Range"),
          caption = "BMI at 6 weeks by primary treatment group.")

```

#### Ethnicity
```{r 6weeks_summary_demographics_ethnicity, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8, eval = TRUE}
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
    dplyr::filter(!is.na(primary_treatment)) %>%
...
```{r 6weeks_summary_demographics_estrogen_receptor, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
    dplyr::filter(!is.na(primary_treatment)) %>%
...
The distribution of provisional histological grade at 6weeks tumor assessment is plotted below.  Whilst this is recorded on both body axes only the most severe is summarised below.

```{r 6weeks_summary_demographics_histo_grade, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
    dplyr::filter(event_name == "6 weeks") %>%
    dplyr::filter(!is.na(primary_treatment)) %>%
    ...
    ylab("N") +
    ggtitle(label = "Provisional Histological Grade at 6 weeks by Primary Treatment Group",
            subtitle = "Y-axis varies between plots") +
    facet_wrap(~primary_treatment, ncol = 1, scales = "free_y") +
    theme_bw() +
    theme(legend.position = "none")
...
spread(test$factor,
    ...
    dplyr::select(-label, -event_name) %>%
    kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
          caption = "Provisional Histological Grade at 6 weeks by Primary Treatment Group.")
...
```{r 6weeks_summary_demographics_eq5d, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of EQ5D at 6 weeks...
##
...
ggplot(eq5d, aes(x = key, fill = Response)) +
    ...
    xlab("") +
    ylab("Proportion") +
    ggtitle("EQ5D Domains at 6 weeks by Primary Treatment") +
...
spread(test$factor,
    ...
    kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
          caption = "EQ5D responses at 6 weeks by Primary Treatment Group.")
## Tabulate the continuous variables
test$continuous %>%
    mutate(Metric = label) %>%
    dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name) %>%
    kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
          caption = "EQ5D Visual Analogue Scale and Score at 6 weeks by Primary Treatment Group.")


```

```{r 6weeks_summary_demographics_eq5d_sjp, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 3,eval = TRUE}
...
#### EORTC QLQ C30
```{r 6weeks_summary_demographics_c30, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of EORTC QLQ C30 at 6 weeks...
##
...
ggplot(c30, aes(x = primary_treatment, fill = Response)) +
    ...
    xlab("") +
    ylab("Proportion") +
    ggtitle(label = "EORTC QLQ C30 Domains at 6 weeks by Primary Treatment",
...
spread(test$factor,
    ...
    arrange(Dimension, primary_treatment) %>%
    kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
          caption = "EORTC QLQ C30 Domains at 6 weeks by Primary Treatment Group.")
## Tabulate the continuous variables
## test$continuous %>%
##     mutate(Metric = label) %>%
##     dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name) %>%
##     kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
##           caption = "EORTC QLQ C30 Scaled Scores at 6 weeks at 6 weeks by Primary Treatment Group.")

```

```{r 6weeks_summary_demographics_c30_sjp, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 3,eval = TRUE}
## Same graphs using sjPlot::sjp.stackfrq(), plot by each treatment.
## To combine use something like cow_plot::plot_grid() / sjPlot::plot_grid() /
## patchwork (see https://github.com/thomasp85/patchwork)
c30_sjp <- list()
...
#### EORTC QLQ BR23
```{r 6weeks_summary_demographics_br23, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of BR23 at 6 weeks...
##
## Histograms of scaled scores by treatment group
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
...
ggplot(br23, aes(x = primary_treatment, fill = Response)) +
    ...
    xlab("") +
    ylab("Proportion") +
    ggtitle(label = "EORTC QLQ BR23 Domains at 6 weeks by Primary Treatment",
            subtitle = "Excludes Missing Responses") +
    facet_wrap(~label, ncol = 2) +
    theme_bw()
## ToDo - Histogram of scores
...
spread(test$factor,
    ...
    arrange(Dimension, primary_treatment) %>%
    kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
          caption = "EORTC QLQ BR23 Domains at 6 weeks by Primary Treatment Group.")
## Tabulate the continuous variables
## test$continuous %>%
##     mutate(Metric = label) %>%
##     dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name) %>%
##     kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
##           caption = "EORTC QLQ BR23 scaled scores at 6 weeks by Primary Treatment Group.")

```

```{r 6weeks_summary_demographics_br23_sjp, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 3,eval = TRUE}
## Same graphs using sjPlot::sjp.stackfrq(), plot by each treatment.
## To combine use something like cow_plot::plot_grid() / sjPlot::plot_grid() /
## patchwork (see https://github.com/thomasp85/patchwork)
br23_sjp <- list()
...
#### EORTC QLQ ELD15
```{r 6weeks_summary_demographics_eld15, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 8,eval = TRUE}
## Generate plots of ELD15 at 6 weeks...
##
## Histograms of scaled scores by treatment group
age_gap %>%
    ## Filter 6 weeks group, with primary_treatment defined and without missing or zero age
...
ggplot(eld15, aes(x = primary_treatment, fill = Response)) +
    ...
    xlab("") +
    ylab("Proportion") +
    ggtitle(label = "EORTC QLQ ELD15 Domains at 6 weeks by Primary Treatment",
            subtitle = "Excludes Missing Responses") +
    facet_wrap(~label, ncol = 2) +
    theme_bw()
## ToDo - Histogram of scores
...
spread(test$factor,
    ...
    arrange(Dimension, primary_treatment) %>%
    kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
          caption = "EORTC QLQ ELD15 Domains at 6 weeks by Primary Treatment Group.")
## Tabulate the continuous variables
## test$continuous %>%
##     mutate(Metric = label) %>%
##     dplyr::select(Metric, primary_treatment, n, missing, mean_sd, median_iqr, range, -event_name) %>%
##     kable(col.names = gsub("primary_treatment", "Primary Treatment", names(.)),
##           caption = "EORTC QLQ ELD15 scaled scores at 6 weeks by Primary Treatment Group.")

```

```{r 6weeks_summary_demographics_eld15_sjp, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 3,eval = TRUE}
## Same graphs using sjPlot::sjp.stackfrq(), plot by each treatment.
## To combine use something like cow_plot::plot_grid() / sjPlot::plot_grid() /
## patchwork (see https://github.com/thomasp85/patchwork)
eld15_sjp <- list()
...
```{r 6weeks_summary_demographics_adl, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 10, eval = TRUE}
## Generate plots of ELD15 at 6 weeks...
##
## Histograms of scaled scores by treatment group
age_gap %>%
...
```{r 6weeks_summary_demographics_adl_sjp, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 3,eval = TRUE}
## Same graphs using sjPlot::sjp.stackfrq(), plot by each treatment.
## To combine use something like cow_plot::plot_grid() / sjPlot::plot_grid() /
## patchwork (see https://github.com/thomasp85/patchwork)
adl_sjp <- list()
...
```{r 6weeks_summary_demographics_iadl, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 10,eval = TRUE}
## Generate plots of ELD15 at 6 weeks...
##
## Histograms of scaled scores by treatment group
age_gap %>%
...
```{r 6weeks_summary_demographics_iadl_sjp, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 3,eval = TRUE}
## Same graphs using sjPlot::sjp.stackfrq(), plot by each treatment.
## To combine use something like cow_plot::plot_grid() / sjPlot::plot_grid() /
## patchwork (see https://github.com/thomasp85/patchwork)
iadl_sjp <- list()
