...
**A Note on Graphs** Currently simple bar charts of numbers (rather tha proportions) are produced (using [`geom_bar()`](http://ggplot2.tidyverse.org/reference/geom_bar.html)), proportions are reported in the tables which will be required anyway.  Alternatives can be produced relatively easily by replacing the desired `geom_###()` and tweaking the `facet_grid()` for plots.
...
master$adverse_events_ae %$%
    table(type, useNA = "ifany") %>%
    kable(col.names = c("Adverse Event Type", "N"),
          caption = "Type of Adverse Events Recorded")

master$adverse_events_ae %$%
    table(ctcae, useNA = "ifany") %>%
    kable(col.names = c("CTCAE Grade", "N"),
          caption = "CTCAE (a measure of severity) of Adverse Events. NA == Not Available i.e. Missing")
...
test <- age_gap %>%
        ...
        ctru::table_summary(df        = .,
                            lookup    = master$lookups_fields,
                            id        = individual_id,
                            select    = c(surgery_aes_acute_gen),
                            time      = event_name,
                            group     = surgery,
                            nomissing = FALSE,
                            digits    = 2,
...
## Print tables of these
dplyr::filter(test$factor,
              label == "Has the participant experienced any acute adverse events relating to breast cancer surgery?") %>%
    spread(surgery,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -value) %>%
    kable(col.names = c("Event", "Response", "Primary", "Adjuvant", "Neoadjuvant"),
          caption   = "Has the participant experienced any acute adverse events relating to breast cancer surgery?")
## Subset and reshape the data for plotting
age_gap %>%
    dplyr::filter(event_name != "Baseline") %>%
    dplyr::filter(surgery == "Yes") %>%
...
test <- age_gap %>%
        dplyr::filter(event_name != "Baseline") %>%
        dplyr::filter(surgery_aes_acute_gen == "Yes") %>%
        ungroup() %>%
        ctru::table_summary(df        = .,
                            lookup    = master$lookups_fields,
                            id        = individual_id,
...
to_plot %>%
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Atelectasis",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
## Table :  Cardiac arrhythmia
test$factor %>%
    dplyr::filter(label == "Cardiac arrhythmia") %>%
    spread(value,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -chemotherapy) %>%
    kable(col.names = c("Event", "No", "Yes", "Missing"),
          caption   = paste0("Cardiac arrhythmia",
                             caption_suffix))
## Plot : Cardiac arrhythmia
to_plot %>%
    dplyr::filter(label == "Cardiac arrhythmia") %>%
    ggplot(aes(response,
               fill = event_name)) +
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Cardiac arrhythmia",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
## Table :  Cardiac Ischemia/infarction
test$factor %>%
    dplyr::filter(label == "Cardiac Ischemia/infarction") %>%
    spread(value,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -chemotherapy) %>%
    kable(col.names = c("Event", "No", "Yes", "Missing"),
          caption   = paste0("Cardiac Ischemia/infarction",
                             caption_suffix))
## Plot : Cardiac Ischemia/infarction
to_plot %>%
    dplyr::filter(label == "Cardiac Ischemia/infarction") %>%
    ggplot(aes(response,
               fill = event_name)) +
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Cardiac Ischemia/infarction",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
## Table :  Cerebrovascular ischemia/stroke
test$factor %>%
    dplyr::filter(label == "Cerebrovascular ischemia/stroke") %>%
    spread(value,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -chemotherapy) %>%
    kable(col.names = c("Event", "No", "Yes", "Missing"),
          caption   = paste0("Cerebrovascular ischemia/stroke",
                             caption_suffix))
## Plot : Cerebrovascular ischemia/stroke
to_plot %>%
    dplyr::filter(label == "Cerebrovascular ischemia/stroke") %>%
    ggplot(aes(response,
               fill = event_name)) +
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Cerebrovascular ischemia/stroke",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
## Table :  Deep vein thrombosis/pulmonary embolism
test$factor %>%
    dplyr::filter(label == "Deep vein thrombosis/pulmonary embolism") %>%
    spread(value,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -chemotherapy) %>%
    kable(col.names = c("Event", "No", "Yes", "Missing"),
          caption   = paste0("Deep vein thrombosis/pulmonary embolism",
                             caption_suffix))
## Plot : Deep vein thrombosis/pulmonary embolism
to_plot %>%
    dplyr::filter(label == "Deep vein thrombosis/pulmonary embolism") %>%
    ggplot(aes(response,
               fill = event_name)) +
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Deep vein thrombosis/pulmonary embolism",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
...
## Table and graphs for each of the possible adverse events by event for
## each of the types of Endocrine Therapy
## Summarise for those who do have adverse events Recorded
test <- age_gap %>%
        dplyr::filter(event_name != "Baseline") %>%
...
## Table and graphs for each of the possible adverse events by event for
## each of the types of Endocrine Therapy
## Summarise the radiotherapy adverse Events.
## Summarise the adverse event repsonses for those who had radiotherapy
test <- age_gap %>%
        dplyr::filter(event_name != "Baseline") %>%
...
test$factor %>%
    ...
    dplyr::select(-label, -radiotherapy) %>%
    ## kable(col.names = c("Event", "No", "Yes", "Missing"),
    kable(col.names = c("Event", "No", "Yes"),
...
## each of the types of Endocrine Therapy
##
## Summarise adverse events for those who experienced one
test <- age_gap %>%
        dplyr::filter(event_name != "Baseline") %>%
...
to_plot %>%
    ...
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
## Table :  Cardiac failure
test$factor %>%
    dplyr::filter(label == "Cardiac failure") %>%
    spread(value,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -trastuzumab) %>%
    ## kable(col.names = c("Event", "No", "Yes", "Missing"),
    kable(col.names = c("Event", "No", "Yes"),
          caption   = paste0("Cardiac failure",
                             caption_suffix))
## Plot : Cardiac failure
to_plot %>%
    dplyr::filter(label == "Cardiac failure") %>%
    ggplot(aes(response,
               fill = event_name)) +
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Cardiac failure",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
...
to_plot %>%
    ...
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
## Table :  Flu-like syndrome
test$factor %>%
    dplyr::filter(label == "Flu-like syndrome") %>%
    spread(value,
           n_prop,
           fill = "0 (0.00)") %>%
    dplyr::select(-label, -trastuzumab) %>%
    ## kable(col.names = c("Event", "No", "Yes", "Missing"),
    kable(col.names = c("Event", "No", "Yes"),
          caption   = paste0("Flu-like syndrome",
                             caption_suffix))
## Plot : Flu-like syndrome
to_plot %>%
    dplyr::filter(label == "Flu-like syndrome") %>%
    ggplot(aes(response,
               fill = event_name)) +
    ...
    xlab("Response") +
    ylab("N") +
    ggtitle(label = paste0("Flu-like syndrome",
                           caption_suffix)) +
    theme_bw() +
    facet_wrap(~event_name, ncol = 1) +
    theme(legend.position = "none")
